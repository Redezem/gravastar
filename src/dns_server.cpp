...
DnsServer::DnsServer(const ServerConfig &config,
...
bool DnsServer::HandleQuery(int sock, const std::vector<unsigned char> &packet,
    ...
    if (blocklist_.IsBlocked(question.qname)) {
        ...
        if (local_records_.Resolve(question.qname, question.qtype, &local_value, &local_type)) {
